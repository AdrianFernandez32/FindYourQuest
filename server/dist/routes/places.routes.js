...
const getEstablishments = async (placeId, type, radius) => {
    try {
        ...
        const response = await axios.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=${radius}&type=${type}&key=${process.env.GOOGLE_API_KEY}`);
        const sortedResults = response.data.results.sort((a, b) => b.rating - a.rating);
