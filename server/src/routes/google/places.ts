...
const getNearbyCities = async (placeId: string) => {
  try {
    const response = await axios.get(
      `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=place_id:${placeId}&radius=50000&type=(cities)&key=${APIKey}`
    );
