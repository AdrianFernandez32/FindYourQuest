...
export async function updateTrip(req: Request, res: Response) {
  ...
  try {
    const user = await conn.query(
      `UPDATE Trip SET start_date='${updatedTrip.start_date}', end_date='${updatedTrip.end_date}', budget=${updatedTrip.budget}, city_name='${updatedTrip.city_id}', flight_in_id=${updatedTrip.flight_in_id}, flight_out_id=${updatedTrip.flight_out_id}, hotel_id=${updatedTrip.hotel_id}`
    );
    return res.json({
